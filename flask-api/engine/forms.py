...
class Compute:
    ...
    def getconfig(self,arg):
        with open(self.config_dir+self.config_file) as fp:
            cond=json.load(fp)
        return(cond[arg])
    ...
    def validate(self):
        ...
        if not(1<self.room_x<=self.getconfig('room_x_max')):
            self.out=f"dimension (x) of the room should be <= than {self.getconfig('room_x_max')} but received {self.room_x})"
        if not(1<self.room_y<=self.getconfig('room_y_max')):
            self.out=f"dimension (y) of the room should be <= than {self.getconfig('room_y_max')} but received {self.room_x})"
    ...
    def calculate(self):
        ...
        try:
            ...
            for d in deg:
            	if d<0:
            		final_angles.append(float(self.full_radius)+d)
            	else:
